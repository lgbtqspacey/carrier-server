...
import { BaseError } from '../tools/CustomError'
  ...
  */
    private static readonly _baseError = (err: BaseError, res: Response) => {
        if (err.isOperational) {
            ...
            Log.error('error_handler', `${err.message}`, err)
            return res.status(err.status).json({ message: ErrorMessage.GENERIC })
        }
    }
   ...
   */
    private static readonly _syntaxError = (err: SyntaxError, res: Response) => {
        Log.error('error_handler', `${err.message}`, err)
        return res.status(HttpStatus.code.BAD_REQUEST).json({ error: ErrorMessage.BAD_REQUEST })
    }
        ...
        if (err.code === MongoDBError.code.DUPLICATE_KEY) {
            ...
            switch (true) {
                case err.message.includes('email'): {
                    return res.status(HttpStatus.code.CONFLICT).json({ error: ErrorMessage.CONFLICT_EMAIL })
                }
                case err.message.includes('id'): {
                    return res.status(HttpStatus.code.CONFLICT).json({ error: ErrorMessage.CONFLICT_ID })
                }
                default: {
                    return res.status(HttpStatus.code.CONFLICT).json({ error: ErrorMessage.CONFLICT })
                }
            }
        } else {
            Log.error('error_handler', `${err.message}`, err)
            return res.status(HttpStatus.code.INTERNAL_SERVER_ERROR).json({ error: ErrorMessage.INTERNAL_SERVER_ERROR })
        }
    }
}
